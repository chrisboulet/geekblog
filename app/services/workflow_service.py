...
def create_workflow_execution(
    ...
    workflow = WorkflowExecution(
        ...
        current_step={"step_name": "initialization", "progress": 0},
    )
    db.add(workflow)
...
def update_workflow_status(
    ...
    """
    workflow = (
        db.query(WorkflowExecution).filter(WorkflowExecution.id == workflow_id).first()
    )
...
def get_workflow_by_id(db: Session, workflow_id: str) -> Optional[WorkflowExecution]:
    """
...
def get_workflow_jobs(db: Session, workflow_id: str) -> List[AsyncJob]:
    """
    Récupérer tous les jobs associés à un workflow
    """
    return (
        db.query(AsyncJob)
        .filter(AsyncJob.workflow_execution_id == workflow_id)
        .order_by(AsyncJob.created_at)
        .all()
    )


def get_active_workflows(db: Session, limit: int = 100) -> List[WorkflowExecution]:
    """
...
def calculate_workflow_progress(db: Session, workflow_id: str) -> float:
    """
