...
def create_project(
    db: Session, project: schemas.ProjectCreate, commit: bool = True
) -> models.Project:
    """
...
def update_project_settings(
    db: Session, project_id: int, settings: Dict[str, Any]
) -> models.Project:
    """
...
def duplicate_project(
    db: Session, project_id: int, new_name: Optional[str] = None
) -> models.Project:
    """
...
def get_all_unique_tags(db: Session) -> List[str]:
    ...
    for project_tags_str in (
        db.query(models.Project.tags).filter(models.Project.tags.isnot(None)).all()
    ):
