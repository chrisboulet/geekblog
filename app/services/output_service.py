...
def save_task_output(
    ...
    existing = db.query(TaskOutput).filter(
        TaskOutput.task_id == task_id,
        TaskOutput.content_hash == content_hash
    ).first()
...
def get_outputs_by_task(
    db: Session,
    task_id: int,
    output_type: Optional[TaskOutputType] = None
) -> List[TaskOutput]:
    """
    Récupérer tous les outputs d'une tâche
    """
    query = db.query(TaskOutput).filter(
        TaskOutput.task_id == task_id
    )
...
def get_outputs_by_workflow(
    ...
    """
    query = db.query(TaskOutput).options(
        joinedload(TaskOutput.task)
    ).filter(
        TaskOutput.workflow_execution_id == workflow_id
    )
...
def cleanup_old_outputs(
    db: Session,
    days_old: int = 30,
    keep_latest_per_task: bool = True
) -> int:
    """
    ...
    if keep_latest_per_task:
        ...
        latest_outputs = db.query(
            TaskOutput.task_id,
            func.max(TaskOutput.created_at).label('max_created')
        ).group_by(TaskOutput.task_id).subquery()
        
        # Supprimer seulement les outputs qui ne sont pas les derniers
        deleted_count = db.query(TaskOutput).filter(
            TaskOutput.created_at < cutoff_date,
            ~db.query(TaskOutput).filter(
