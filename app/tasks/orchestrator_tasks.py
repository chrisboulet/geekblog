...
from app.tasks.ai_tasks import planning_task, research_task
...
@celery_app.task(
    bind=True,
    base=JobAwareTask,
    name="app.tasks.orchestrator_tasks.full_article_workflow_task",
)
def full_article_workflow_task(
    self, project_id: int, workflow_execution_id: str
) -> dict:
    """
    ...
    try:
        ...
        with get_db() as db:
            workflow_service.update_workflow_status(
                ...
                error_details={"error": str(e), "step": "workflow_initialization"},
            )
        raise
...
@celery_app.task(
    bind=True,
    base=JobAwareTask,
    name="app.tasks.orchestrator_tasks.research_coordinator_task",
)
def research_coordinator_task(
    self, planning_result: dict, workflow_execution_id: str
) -> dict:
    """
    ...
    try:
        ...
        if not planning_result.get("success", False):
            raise ValueError(
                f"Planning failed: {planning_result.get('message', 'Unknown error')}"
            )
        ...
        with get_db() as db:
            workflow_service.update_workflow_status(
                ...
                workflow_id=workflow_execution_id,
                status=WorkflowStatus.FAILED,
                error_details={"error": str(e), "step": "research_coordination"},
            )
        raise
...
@celery_app.task(
    bind=True,
    base=JobAwareTask,
    name="app.tasks.orchestrator_tasks.research_completion_task",
)
def research_completion_task(
    self, research_results: List[dict], workflow_execution_id: str
) -> dict:
    """
    ...
    try:
            ...
            workflow_service.update_workflow_step(
                ...
                metadata={
                    ...
                    "successful_tasks": len(
                        [r for r in research_results if r.get("success", False)]
                    ),
                },
            )
        ...
        with get_db() as db:
            workflow_service.update_workflow_status(
                ...
                workflow_id=workflow_execution_id,
                status=WorkflowStatus.FAILED,
                error_details={"error": str(e), "step": "research_completion"},
            )
        raise


@celery_app.task(
    bind=True, base=JobAwareTask, name="app.tasks.orchestrator_tasks.assembly_task"
)
def assembly_task(
    self, research_completion_result: dict, project_id: int, workflow_execution_id: str
) -> dict:
    """
    ...
    try:
        ...
        with get_db() as db:
            ...
            assembled_content = output_service.merge_outputs_for_assembly(
                db=db, task_ids=task_ids, separator="\n\n---\n\n"
            )
        ...
        with get_db() as db:
            workflow_service.update_workflow_status(
                ...
                workflow_id=workflow_execution_id,
                status=WorkflowStatus.FAILED,
                error_details={"error": str(e), "step": "assembly"},
            )
        raise
...
@celery_app.task(
    bind=True,
    base=JobAwareTask,
    name="app.tasks.orchestrator_tasks.project_finishing_task",
)
def project_finishing_task(
    self, assembly_result: dict, project_id: int, workflow_execution_id: str
) -> dict:
    """
    ...
    try:
        ...
        with get_db() as db:
            ...
            workflow_service.mark_workflow_complete(
                db=db, workflow_id=workflow_execution_id, success=True
            )
        ...
        with get_db() as db:
            workflow_service.mark_workflow_complete(
                ...
                workflow_id=workflow_execution_id,
                success=False,
                error_details={"error": str(e), "step": "finishing"},
            )
        raise
