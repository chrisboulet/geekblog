...
from app.services import (
    ...
    task_service,
    ai_service,
)  # Ajout de task_service et ai_service
...
@router.post(
    ...
    tags=["Projects", "AI Planning"],
)
async def plan_project_with_ai(
    project_id: int,
    project_goal: Optional[str] = Body(
        None,
        description="Objectif détaillé du projet pour guider la planification IA. Si non fourni, la description du projet sera utilisée.",
    ),
    ...
    if not goal_to_plan:
        raise HTTPException(
            status_code=400,
            detail="Project goal or description is required for AI planning.",
        )

    try:
        task_titles = await ai_service.run_planning_crew_async(goal_to_plan)
    ...
    order_start = len(
        db_project.tasks
    )  # Pour que les nouvelles tâches soient ajoutées à la fin
...
@router.post(
    "/{project_id}/plan-async",
    response_model=JobStatus,
    tags=["Projects", "AI Planning", "Async"],
)
async def plan_project_async(
    ...
    job_service.create_job_record(
        db=db, job_id=job.id, job_type="planning", project_id=project_id
    )
...
class AssemblePayloadBody(
    schemas.BaseModel
):  # Renommé pour éviter conflit avec un potentiel type AssemblePayload de l'API
    raw_content: str


@router.post(
    "/{project_id}/assemble", response_model=str, tags=["Projects", "AI Finishing Crew"]
)  # Réponse est le texte raffiné
async def assemble_and_refine_project_content(
    ...
    if not payload.raw_content or not payload.raw_content.strip():
        raise HTTPException(
            status_code=400, detail="Raw content for assembly cannot be empty."
        )

    try:
        refined_article = await ai_service.run_finishing_crew_async(payload.raw_content)
    except EnvironmentError as e:
        raise HTTPException(status_code=503, detail=f"AI Service Unavailable: {e}")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"AI finishing crew failed: {str(e)}"
        )

    if not refined_article:
        raise HTTPException(
            status_code=500, detail="AI finishing crew did not return any content."
        )
...
@router.post(
    "/{project_id}/assemble-async",
    response_model=JobStatus,
    tags=["Projects", "AI Finishing Crew", "Async"],
)
async def assemble_and_refine_project_content_async(
    project_id: int, payload: AssemblePayloadBody, db: Session = Depends(get_db)
):
    """
    ...
    # Créer l'enregistrement en base de données
    job_service.create_job_record(
        db=db, job_id=job.id, job_type="finishing", project_id=project_id
    )
...
def update_project_endpoint(
    ...
    db_project = project_service.update_project(
        db, project_id=project_id, project_update=project
    )
    if db_project is None:
        raise HTTPException(status_code=404, detail="Project not found")
    return db_project
...
@router.post(
    "/{project_id}/workflows/generate-article",
...
async def launch_article_workflow(
    ...
    if workflow_data.project_id != project_id:
        raise HTTPException(
            status_code=400, detail="Project ID mismatch in URL and body"
        )
    ...
    try:
        ...
        workflow_job = full_article_workflow_task.delay(
            project_id, workflow_execution.id
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Échec du lancement du workflow: {str(e)}"
        )
...
@router.get(
    "/workflows/{workflow_id}/outputs",
    response_model=WorkflowOutputsResponse,
    tags=["Workflows"],
)
async def get_workflow_outputs(workflow_id: str, db: Session = Depends(get_db)):
    """
    ...
    for output in raw_outputs:
        summary = TaskOutputSummary(
            ...
            word_count=output.metadata.get("word_count")
            if output.metadata
            else len(output.content.split()),
