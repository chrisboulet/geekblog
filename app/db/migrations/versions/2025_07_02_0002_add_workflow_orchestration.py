...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_workflow_project_status",
        "workflow_executions",
        ["project_id", "status"],
        unique=False,
    )
    op.create_index(
        "idx_workflow_started", "workflow_executions", ["started_at"], unique=False
    )
    ...
    op.create_index(
        "idx_output_task_type", "task_outputs", ["task_id", "output_type"], unique=False
    )
    op.create_index(
        "idx_output_workflow", "task_outputs", ["workflow_execution_id"], unique=False
    )
    ...
    op.add_column(
        "async_jobs", sa.Column("workflow_execution_id", sa.String(), nullable=True)
    )
    ...
    op.create_index(
        "idx_async_jobs_workflow", "async_jobs", ["workflow_execution_id"], unique=False
    )
    op.create_index(
        "idx_async_jobs_parent", "async_jobs", ["parent_job_id"], unique=False
    )
    ...
    op.add_column(
        "projects",
        sa.Column(
            "final_content_updated_at", sa.DateTime(timezone=True), nullable=True
        ),
    )
