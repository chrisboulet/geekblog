...
from app.models.models import BlogTemplate, Task
...
class TestTemplateService:
    ...
    def test_create_template(self, db_session: Session):
        ...
        template_data = BlogTemplateCreate(
            ...
            sample_expressions={
                "moyen": {"opener": "Test opener", "closer": "Test closer"}
            },
        )
    ...
    def test_get_template_by_id(
        self, db_session: Session, sample_template: BlogTemplate
    ):
    ...
    def test_get_template_by_slug(
        ...
        template = template_service.get_template_by_slug(
            db_session, sample_template.slug
        )
    ...
    def test_update_template(self, db_session: Session, sample_template: BlogTemplate):
        ...
        updated = template_service.update_template(
            db_session, sample_template.id, update_data
        )
    ...
    def test_deactivate_template(
        ...
        deactivated = template_service.deactivate_template(
            db_session, sample_template.id
        )
...
class TestProjectFromTemplate:
    ...
    def test_create_project_from_guide_pratique(self, db_session: Session):
        ...
        project = template_service.create_project_from_template(
            db_session, template.id, customization
        )
    ...
    def test_create_project_invalid_template(self, db_session: Session):
        ...
        customization = TemplateCustomization(
            title="Test", theme="Test", localization_level="moyen", audience="québécois"
        )
    ...
    def test_create_project_with_custom_instructions(
        self, db_session: Session, sample_template: BlogTemplate
    ):
        ...
        project = template_service.create_project_from_template(
            db_session, sample_template.id, customization
        )
...
def sample_template(db_session: Session) -> BlogTemplate:
    ...
    template = BlogTemplate(
        ...
        sample_expressions={"moyen": {"test": "expression"}},
        is_active=True,
    )
    db_session.add(template)
    db_session.commit()
