...
class TestAIServiceAgents:
    ...
    def test_run_research_crew_success(self, mock_crew, mock_llm):
        ...
        with patch.object(
            ai_service, "run_research_crew", return_value=mock_result
        ) as mock_research:
            result = ai_service.run_research_crew(
                "Docker containers", "Context about containers"
            )
    ...
    def test_run_writing_crew_success(self, mock_crew, mock_llm):
        ...
        with patch.object(
            ai_service, "run_writing_crew", return_value=mock_result
        ) as mock_writing:
            result = ai_service.run_writing_crew(
                "Introduction Docker", "Research context"
            )
            assert result == mock_result
            mock_writing.assert_called_once_with(
                "Introduction Docker", "Research context"
            )
...
class TestAIServiceFinishingCrew:
    ...
    def test_run_finishing_crew_success(self, mock_crew, mock_llm):
        ...
        with patch.object(
            ai_service, "run_finishing_crew", return_value=refined_content
        ) as mock_finishing:
    ...
    def test_run_finishing_crew_no_llm(self):
        ...
        with patch.object(
            ai_service,
            "run_finishing_crew",
            side_effect=EnvironmentError("LLM non initialisé"),
        ) as mock_finishing:
...
class TestAIServiceConfiguration:
    ...
    def test_llm_initialization_success(self, mock_chatgroq):
        ...
        mock_chatgroq.assert_called_with(
            api_key="test_key", model="llama3-8b-8192", temperature=0.7
        )
    ...
    def test_search_tool_initialization(self):
        ...
        assert hasattr(ai_service.search_tool, "run") or hasattr(
            ai_service.search_tool, "_run"
        )
