...
class TestProjectArchiving:
    ...
    def test_archive_project_success(self, db: Session, sample_project: models.Project):
        ...
        archived_project = project_service.archive_project(
            db=db, project_id=sample_project.id, reason=reason
        )
    ...
    def test_archive_already_archived_project(
        self, db: Session, archived_project: models.Project
    ):
    ...
    def test_unarchive_project_success(
        ...
        unarchived_project = project_service.unarchive_project(
            db=db, project_id=archived_project.id
        )
    ...
    def test_unarchive_active_project(
        self, db: Session, sample_project: models.Project
    ):
...
class TestProjectSettings:
    ...
    def test_get_project_settings_empty(
        self, db: Session, sample_project: models.Project
    ):
        """Test de récupération des paramètres d'un projet sans paramètres"""
        settings = project_service.get_project_settings(
            db=db, project_id=sample_project.id
        )
        assert settings == {}

    def test_update_project_settings_new(
        self, db: Session, sample_project: models.Project
    ):
        ...
        updated_project = project_service.update_project_settings(
            db=db, project_id=sample_project.id, settings=new_settings
        )

        assert updated_project is not None
        assert updated_project.settings == new_settings

    def test_update_project_settings_merge(
        self, db: Session, project_with_settings: models.Project
    ):
        ...
        updated_project = project_service.update_project_settings(
            db=db, project_id=project_with_settings.id, settings=additional_settings
        )
        ...
        assert updated_project.settings["custom_prompts"] == {
            "intro": "Commencez par..."
        }  # Added
...
class TestProjectDuplication:
    ...
    def test_duplicate_project_default_name(
        self, db: Session, sample_project_with_tasks: models.Project
    ):
        """Test de duplication avec nom par défaut"""
        duplicated_project = project_service.duplicate_project(
            db=db, project_id=sample_project_with_tasks.id
        )
    ...
    def test_duplicate_project_custom_name(
        self, db: Session, sample_project: models.Project
    ):
        ...
        duplicated_project = project_service.duplicate_project(
            db=db, project_id=sample_project.id, new_name=custom_name
        )
...
class TestProjectFiltering:
    ...
    def test_get_projects_filtered_exclude_archived(
        ...
        projects = project_service.get_projects_filtered(db=db, include_archived=False)
    ...
    def test_get_projects_filtered_include_archived(
        ...
        projects = project_service.get_projects_filtered(db=db, include_archived=True)

        project_ids = [p.id for p in projects]
        assert sample_project.id in project_ids
        assert archived_project.id in project_ids

    def test_get_projects_filtered_by_tags(
        self, db: Session, project_with_tags: models.Project
    ):
        """Test de filtrage par tags"""
        projects = project_service.get_projects_filtered(db=db, tags="blog")

        assert len(projects) > 0
        assert project_with_tags.id in [p.id for p in projects]

    def test_get_projects_filtered_pagination(
        self, db: Session, multiple_projects: list
    ):
        """Test de pagination"""
        # Premier batch
        first_batch = project_service.get_projects_filtered(db=db, skip=0, limit=2)

        # Deuxième batch
        second_batch = project_service.get_projects_filtered(db=db, skip=2, limit=2)
...
class TestProjectDeletionSafety:
    ...
    def test_delete_archived_project_fails(
        self, db: Session, archived_project: models.Project
    ):
    ...
    def test_delete_active_project_success(
        self, db: Session, sample_project: models.Project
    ):
        ...
        remaining_project = project_service.get_project(
            db=db, project_id=sample_project.id
        )
