...
class TestTasksCRUD:
    ...
    def test_create_task_invalid_project(self, client: TestClient):
        ...
        task_data = {"project_id": 99999, "title": "Tâche orpheline"}
...
class TestTasksByProject:
    ...
    def test_get_tasks_for_project_success(
        self, client: TestClient, sample_project: Project, multiple_tasks: list[Task]
    ):
    ...
    def test_get_tasks_for_empty_project(
        self, client: TestClient, sample_project: Project
    ):
    ...
    def test_get_tasks_with_pagination(
        ...
        # Test avec skip
        response = client.get(
            f"/api/v1/tasks/project/{sample_project.id}?skip=1&limit=2"
        )
        assert response.status_code == 200
        tasks = response.json()
        assert len(tasks) == 2
...
class TestTasksAIEndpoints:
    ...
    def test_run_agent_valid_types(self, client: TestClient, sample_task: Task):
        ...
        # Test avec writer
        response = client.post(
            f"/api/v1/tasks/{sample_task.id}/run-agent", json={"agent_type": "writer"}
        )
        assert response.status_code == 503
    ...
    def test_run_agent_task_not_found(self, client: TestClient):
        ...
        response = client.post(
            "/api/v1/tasks/99999/run-agent", json={"agent_type": "researcher"}
        )
...
class TestTasksValidation:
    ...
    def test_task_empty_title(self, client: TestClient, sample_project: Project):
        ...
        task_data = {"project_id": sample_project.id, "title": ""}
    ...
    def test_task_very_long_title(self, client: TestClient, sample_project: Project):
        ...
        long_title = "x" * 1000
        task_data = {"project_id": sample_project.id, "title": long_title}
